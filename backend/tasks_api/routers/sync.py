...
async def _full_list_state(engine: CalDAVEngine, info: ListInfo) -> tuple[str, list[Task]]:
    ...
    tasks = [t for s in states if (t := _task_from_state(s, info.id)) is not None]
    return token, tasks
...
async def _full_snapshot(engine: CalDAVEngine, lists: list[ListInfo]) -> SyncResponse:
    ...
    tasks: list[Task] = []
    for info in lists:
        token, list_tasks = await _full_list_state(engine, info)
        tokens[info.id] = token
        tasks.extend(list_tasks)
    return SyncResponse(
        cursor=cursor_codec.encode_cursor(tokens),
        full=True,
...
async def _delta(
    engine: CalDAVEngine, lists: list[ListInfo], old_tokens: dict[str, str]
) -> SyncResponse:
    tokens: dict[str, str] = {}
    tasks: list[Task] = []
    ...
    for info in lists:
        ...
        tasks.extend(
            t for s in delta.changed if (t := _task_from_state(s, info.id)) is not None
        )
        tasks.extend(
            _tombstone(CalDAVEngine.uid_hint_from_href(href), info.id)
...
async def sync(
    ...
    if old_tokens is None:
        ...
        try:
            response = await _delta(engine, lists, old_tokens)
